(function () {
  ...
  //
  // The overlay uses position:fixed so it never affects document flow or
  // causes the TDX layout/navShim to shift during load.
  //
  // Usage: /SBTDClient/413/Portal/Requests/Service/12357/Test-Release-Ticket/Request?prefill=TICKET_ID
  (function () {
    ...
    // ── Loading overlay — position:fixed so it never shifts document flow ─────
    // Sits above all TDX content (z-index 998) but below the non-prod warning
    // banner (z-index 999999). Covers the full viewport during fetch.
    ...
    function showPrefillBanner(ticketId, populated, failed, errorMsg) {
      ...
      var ticketLink = 'ticket <a href="/SBTDClient/413/Portal/Requests/TicketRequests/TicketDet?TicketID=' + ticketId + '" style="color: inherit; text-decoration: underline;">#' + ticketId + '</a>';
