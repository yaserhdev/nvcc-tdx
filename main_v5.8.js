(function () {
  ...
  //
  // Detection: the banner iframe sets its own style.height after every state
  // change. We watch all iframes for a height change — that signals the banner
  // has settled into its final rendered state.
  //
  ...
  window.nvccShowUserDetailsOverlay = function () {
    ...
    // Watch iframes for height change FROM their initial value —
    // the banner iframe starts with a fixed inline height (214px) set by TDX,
    // then gets resized by the banner's own resize script once content renders.
    // We wait for that dynamic resize, not just any non-zero height.
    function watchIframes() {
      document.querySelectorAll('iframe').forEach(function (iframe) {
        ...
        var mo = new MutationObserver(function () {
          ...
          // Only complete when height has changed from initial AND is non-zero
          if (currentHeight !== initialHeight && parseInt(currentHeight) > 0) {
            mo.disconnect();
            removeOverlay();
          }
        });
