(function () {
  ...
  // The MutationObserver watches the datepicker div and hides it if it opens
  // before any user interaction, then blurs the input after a setTimeout(0) so
  // jQuery UI's internal state is fully reset and the first click works normally.
  ...
  // MutationObserver safety net — watches the datepicker div and hides it if
  // it opens before the user has interacted. Defers the blur via setTimeout(0)
  // so jQuery UI finishes its open sequence before we reset focus state —
  // without this, the first click won't re-open the calendar.
  (function () {
    var dpDiv = document.getElementById('ui-datepicker-div');
