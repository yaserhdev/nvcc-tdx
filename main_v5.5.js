(function () {
  ...
  // Prevent any element from receiving focus on page load by injecting a style
  // that removes pointer-events briefly, then restoring after load. The simpler
  // approach: just blur whatever has focus at every possible load timing point.
  // Uses a CSS trick to suppress the blinking cursor on initial load without
  // disrupting jQuery UI's click-to-open behavior.
  (function () {
    // Inject a style that prevents focus-on-load for all inputs by setting
    // them to pointer-events none briefly — we re-enable on first interaction
    var noFocusStyle = document.createElement('style');
    noFocusStyle.id = 'nvcc-no-autofocus';
    noFocusStyle.textContent = '* { -webkit-user-select: none !important; }';
    document.head.appendChild(noFocusStyle);

    function suppressAutoFocus() {
    ...
    // Run at every possible timing
    suppressAutoFocus();
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', suppressAutoFocus);
    }
    window.addEventListener('load', suppressAutoFocus);
    setTimeout(suppressAutoFocus, 0);
    setTimeout(suppressAutoFocus, 100);
    setTimeout(suppressAutoFocus, 300);
    setTimeout(suppressAutoFocus, 500);

    // Remove the no-select style on first real user interaction
    function onFirstInteraction() {
      var s = document.getElementById('nvcc-no-autofocus');
    ...
    document.addEventListener('mousedown', onFirstInteraction, { capture: true });
