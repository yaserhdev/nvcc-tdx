(function () {
  ...
  //
  // Detection: watches all iframes for height changes via MutationObserver.
  // Dismisses on the second height change (first = loading state, second =
  // final rendered state). Safety timeout at 8s removes overlay regardless.
  //
  // Usage: call window.nvccShowFormOverlay() from any form IIFE after initial
  // field setup, before or alongside any iframe-based widget embed.
  window.nvccShowFormOverlay = function () {
    ...
    // Watch iframes for height changes. Dismiss on second change.
  ...
  // Injects release ticket cards above the form fields via an iframe widget
  // hosted on a Work Management public dashboard. The universal form overlay
  // covers the page while the iframe renders its content.
  (function () {
    var formIdEl = document.getElementById('FormID');
    if (!formIdEl || formIdEl.value !== '12356') return;

    // Show universal overlay while release notes iframe loads
    window.nvccShowFormOverlay();
