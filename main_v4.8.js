(function () {
  ...
  // ─── DATEPICKER AUTOFOCUS PREVENTION ─────────────────────────────────────────
  // On TDX form pages, if a datepicker field is the first focusable input it
  // receives focus automatically on load, which immediately opens the calendar
  // widget before the user has interacted with anything.
  // Fix: on DOMContentLoaded and again on load (to catch late jQuery UI init),
  // blur any datepicker that has focus and close the calendar widget if open.
  // The double-pass handles both early and late jQuery UI datepicker init timing.
  if (document.querySelector('form.js-entry-form')) {
    var blurDatepicker = function () {
      var focused = document.activeElement;
      if (focused && focused.classList.contains('datepicker')) {
        focused.blur();
        // Close via jQuery UI API if available — belt and suspenders
        if (window.jQuery && window.jQuery(focused).datepicker) {
          try { window.jQuery(focused).datepicker('hide'); } catch (e) {}
        }
      }
      // Also close any already-open datepicker widget regardless of focus
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', blurDatepicker);
    } else {
      blurDatepicker();
    }
    // Second pass on window load catches cases where jQuery UI init fires late
    window.addEventListener('load', blurDatepicker);
  }
