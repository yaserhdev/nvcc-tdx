(function () {
  ...
  // ─── SEARCH BAR HIDE ─────────────────────────────────────────────────────────
  // Hides the header search bar on home pages which have their own search bar.
  (function () {
  ...
  // ─── GLOBAL STYLES ───────────────────────────────────────────────────────────
  // Navbar font, active pill, hover states, navshim sync.
  (function () {
    ...
    // NavShim sync — keeps the content shim in sync with the fixed header height
  ...
  // ─── AUTOFOCUS FIX ───────────────────────────────────────────────────────────
  // Prevents TDX from auto-focusing datepicker fields on load, which opens the
  // calendar widget unexpectedly and requires an extra click to dismiss.
  (function () {
  ...
  // ─── HIDE LAST MODIFIED ──────────────────────────────────────────────────────
  // Hides the "Last Modified" timestamp on ticket detail pages for specific
  // services listed in CONFIG.hideLastModified.
  (function () {
  ...
  // ─── HR OL&D PAGE ────────────────────────────────────────────────────────────
  // Replaces the native TDX services list on the OL&D category page with a
  // custom styled content block and sidebar shortcut buttons.
  (function () {
  ...
  // ─── FORM OVERLAY ────────────────────────────────────────────────────────────
  // Shows a loading overlay on any form page that contains an iframe widget
  // (user details banner, release notes cards, etc.). Dismisses automatically
  // when the widget iframe sends a nvcc-widget-ready postMessage signal.
  //
  // Every HTML module widget should include this universal notify snippet and
  // call nvccNotifyParentReady() at the exact point content is fully loaded:
  //
  //   var _nvccNotified = false;
  //   function nvccNotifyParentReady() {
  //     if (_nvccNotified) return;
  //     _nvccNotified = true;
  //     var t = window.top || window.parent;
  //     if (t && t !== window) t.postMessage({ type: 'nvcc-widget-ready' }, '*');
  //   }
  (function () {

    // Early postMessage listener — registered immediately to avoid race
    // conditions where the widget sends nvcc-widget-ready before the overlay
    // has been created and set up its callback.
    var _widgetReadyCallback = null;
    var _widgetReadyQueued   = false;
    ...
    window.addEventListener('message', function (event) {
      ...
      if (typeof _widgetReadyCallback === 'function') {
        ...
        _widgetReadyQueued = true;
      }
    });

    // nvccShowFormOverlay — call this to show the overlay on any form page.
    // Guards against double-call so only one overlay exists at a time.
    window.nvccShowFormOverlay = function () {
      ...
      overlay.innerHTML = [
        '<div style="text-align:center;">',
          '<div style="',
            ...
            'border:3px solid #d4e6da;',
            'border-top-color:#046a38;',
            'border-radius:50%;',
            'animation:nvcc-spin 0.7s linear infinite;',
            'margin-bottom:14px;',
          '"></div>',
          '<style>@keyframes nvcc-spin{to{transform:rotate(360deg);}}</style>',
          '<div style="color:#046a38;font-size:15px;font-weight:600;">Loading form content...</div>',
        '</div>'
      ].join('');
      ...
      // If widget already fired before overlay was created, dismiss immediately
      if (_widgetReadyQueued) {
        _widgetReadyQueued = false;
        removeOverlay();
        return;
      }
    ...
    // Auto-detect: show overlay on any form page that has an iframe
  ...
  // Hides remaining fields until a valid start date is entered.
  // Rules: future date, at least 5 business days out, no more than 30 days out.
  (function () {
    function init() {
  ...
  // ─── FORM PREFILL (DISABLED) ──────────────────────────────────────────────────
  // Triggered when URL contains ?prefill=TICKET_ID. Fetches the ticket from
  // TDNext and populates matching form fields. Configuration lives in a hidden
  // static content field on the form as JSON:
  //
  ...
  //     document.body.appendChild(overlay);
  //
  //     var barEl = overlay.querySelector('#nvcc-prefill-bar');
  //     var pctEl = overlay.querySelector('#nvcc-prefill-pct');
  //
  //     function animatePct() {
