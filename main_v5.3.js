(function () {
  ...
  //     // Never touch datepicker inputs — any interaction (blur or datepicker API)
  //     // disrupts jQuery UI's internal focus binding, causing the calendar to not
  //     // open on the first click. Instead, just hide the widget div directly and
  //     // leave the input element's focus state untouched.
  //     var isDatepicker = focused.classList && focused.classList.contains('datepicker');
  //     if (!isDatepicker) {
  //       focused.blur();
  //     }
  //   }
  //   // Hide the datepicker widget div directly — works regardless of focus state
  ...
  // Extra delayed passes — catches datepicker opened by async content (e.g.
  // iframes or late-initializing widgets) that fire after window load.
  setTimeout(blurAutoFocus, 100);
  setTimeout(blurAutoFocus, 300);
  setTimeout(blurAutoFocus, 600);

  // MutationObserver safety net — if the datepicker div becomes visible at any
  // point shortly after load (before the user has interacted), hide it immediately.
  // Disconnects after 3 seconds so it doesn't interfere with normal usage.
  (function () {
    var dpDiv = document.getElementById('ui-datepicker-div');
    ...
    var observer = new MutationObserver(function () {
      if (!userInteracted && dpDiv.style.display !== 'none') {
        dpDiv.style.display = 'none';
        // Also blur the focused datepicker so jQuery UI resets its internal
        // state — without this the calendar won't open on the first click
        var focused = document.activeElement;
        if (focused && focused.classList && focused.classList.contains('datepicker')) {
          focused.blur();
        }
      }
    });
  ...
  //   - Submit button is disabled whenever any validation error is active or
  //     fields are hidden; enabled only when all conditions pass.
  ...
  (function () {
    ...
    // Excludes static-2997048-grp (the section heading) which is not an input.
    var HIDDEN_FIELD_GROUPS = [
      ...
      'divButtons',           // Submit button container
    ];
    ...
    // Option 1: disable/enable the real TDX submit button directly.
    ...
    function validateEndDate() {
      ...
      if (!endDate || !startDate) {
        clearError(errorId);
        // Re-enable submit only if remaining fields are visible (start date valid)
        // — end date being empty is not itself an error
