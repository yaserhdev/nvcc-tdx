(function () {
  ...
  var blurAutoFocus = function () {
    ...
    if (focused && focused !== document.body && focused !== document.documentElement) {
      // For datepicker inputs: close the widget via jQuery UI API rather than
      // blurring the element — blur() detaches the datepicker's focus binding,
      // which causes the calendar to not open on the first click after page load.
      if (focused.classList && focused.classList.contains('datepicker') && window.jQuery) {
        try { window.jQuery(focused).datepicker('hide'); } catch (e) {}
      } else {
        focused.blur();
      }
    }
    // Belt-and-suspenders: hide the widget div directly in case it rendered
    // before the jQuery UI instance was fully bound
