(function () {
  ...
  window.nvccShowFormOverlay = function () {
    ...
    // Listen for postMessage from widget iframes. Each widget sends
    // { type: 'nvcc-widget-ready' } once it has fully rendered its final
    // state (content, error, or empty). This is more reliable than height
    // detection since it fires at the exact moment the widget is done,
    // regardless of content size, pre-rendering, or timing.
    var messageHandler = function (event) {
      if (event.data && event.data.type === 'nvcc-widget-ready') {
